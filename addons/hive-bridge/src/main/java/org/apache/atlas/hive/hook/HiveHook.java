...
public class HiveHook extends AtlasHook implements ExecuteWithHookContext {
    ...
    public void run(HookContext hookContext) throws Exception {
        ...
        LineageInfo linfo=queryState.getLineageState().getLineageInfo();

        LOG.info("atlas   run  entrySet size  ---------------------------"+linfo.entrySet().size());


        LOG.info("atlas run ---------------------");
        LOG.info("hookContext.getLinfo().entrySet().size()="+hookContext.getLinfo().entrySet().size());
        LOG.info("atlas run ++++++++++++++++++++++++");
